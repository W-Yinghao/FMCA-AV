...
DOWNLOAD_RUN="20260807-032855_dataset-robustness-relocated"
...
wait_download() {
  local state
  while true; do
    python3 -m harness.cli status --run "$DOWNLOAD_RUN" >/dev/null
    state="$($PYTHON -c 'import json,sys; print(json.load(open(sys.argv[1]))["state"])' "runs/$DOWNLOAD_RUN/status.json")"
    case "$state" in
      SUCCEEDED) return 0 ;;
      FAILED|STOPPED|BLOCKED) echo "$DOWNLOAD_RUN ended in $state" >&2; return 1 ;;
      *) sleep 300 ;;
    esac
  done
}
...
for suite in imagenet-c imagenet-r imagenet-a; do
  ...
  command+=" --checkpoint $checkpoint --probe-checkpoint $probe_checkpoint"
  command+=" --root /projects/EEG-foundation-model/yinghao/FMCA-AV/robustness --suite $suite"
  command+=' --output "$FMCA_HARNESS_RUN_DIR/artifacts/imagenet_robustness.json"'
  command+=" --device cuda --batch-size 256 --workers 8"
  run_id="$(submit_with_capacity_wait --name "imagenet1k-32step-${suite}-robustness" \
