#!/usr/bin/env python3
"""Capacity-aware CIFAR-10 frozen gradient-variance experiment and renderer."""
...
def submit() -> str:
    command = [
        "python3", "-m", "harness.cli", "submit", "--name", "e2-cifar10-frozen-gradient-variance-500",
        ...
        "--checkpoint", CHECKPOINT, "--repetitions", "500",
    ]
    while True:
...
def wait_success(run_id: str) -> None:
    while True:
        ...
        if value in {"FAILED", "STOPPED", "BLOCKED"}: raise RuntimeError(f"E2 CIFAR gradient run {run_id} ended in {value}")


def main() -> int:
    artifacts = Path(os.environ["FMCA_HARNESS_RUN_DIR"]) / "artifacts"; artifacts.mkdir(parents=True, exist_ok=True)
    time.sleep(POLL_SECONDS); run_id = submit(); (artifacts / "gradient_run.txt").write_text(run_id + "\n", encoding="utf-8")
    wait_success(run_id)
