# Plug-in 研究预注册（FROZEN 2026-08-20）
...
- 数据/预算：v8 满配树、M=8 端点、200 epochs、lr 与优化器沿用各 base 的 formal 设置（裁决：全行统一 SGD 0.1 + warmup10 + cosine，与 v8 gate 相同，保持行间匹配；该研究不依赖 v8 判定，已随冻结启动：双 QC probe 952219/952220 + 18 单元 fleet）。
