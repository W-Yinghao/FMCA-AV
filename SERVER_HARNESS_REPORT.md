...
- harness 全局硬上限：6 GPU；单任务硬上限：2 GPU；允许请求固定为 `0,1,2`。`QUEUED` 与 `RUNNING` 请求一起计入全局上限。
...
python3 -m harness.cli dry-run --name <name> --gpus <0|1|2> --profile default -- <command> [args...]
python3 -m harness.cli submit --name <name> --gpus <0|1|2> --profile default -- <command> [args...]
python3 -m harness.cli dry-run --name <imagenet-name> --gpus <1|2> --profile imagenet -- <command> [args...]
python3 -m harness.cli submit --name <imagenet-name> --gpus <1|2> --profile imagenet -- <command> [args...]
...
| 5 | 拒绝 4/5 GPU | PASS | argparse 以退出码 2 拒绝，只接受 `0,1,2`。 |
| 6 | 全局 6-GPU 运行时拒绝额外 1 GPU | PASS | 三个 2-GPU 任务或六个 1-GPU 任务已占满时，额外请求会立即拒绝；任何单任务 4-GPU 请求也会在参数解析阶段拒绝。 |
...
| 10 | 2-GPU DDP dry-run | PASS | `python train.py` 正确转换为单节点 `torchrun --standalone --nnodes=1 --nproc_per_node=2 ...`；不匹配进程数、多节点命令及 4-GPU 请求均被拒绝。 |
...
- 历史 4-GPU 验收项已由新的单任务 2-GPU 上限取代；历史 run 仅保留审计信息。

## 9. 下一条建议指令（未执行）

所有后续任务通过 harness 的 `sbatch` 路径运行：项目总并发不超过 6 GPU，单任务不超过 2 GPU。ImageNet 正式训练优先使用 2×H100。
