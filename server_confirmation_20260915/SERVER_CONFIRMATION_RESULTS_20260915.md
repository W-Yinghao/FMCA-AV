...
**新里程碑复现（`gate1_20260914_ridge_ms_beta0`、`_c100`）**：**6/6 完成（2026-09-15 15:40 更新）**，记录见 `RIDGE_BETA0_MILESTONE_RESULTS_20260915.md`。P-C1 确认：六个 epoch-200 区间全部与归档 β=0 区间重叠（C10 SGD [83.42, 83.58] / st2 [79.55, 79.92] / st4 [83.19, 83.53]；C100 [54.79, 55.55] / [53.75, 53.98] / [54.90, 55.30]）。第 20 epoch 逐层评价：C10 β=0 [56.11–56.87 / 67.11–67.81 / 68.83–70.10 / 68.15–69.89]，C100 β=0 [29.84–30.71 / 40.73–41.70 / 41.78–44.14 / 40.77–43.12]。与同批完整模型复现按 seed 配对的均值、SD、成对差已追加进 `ridge_beta0_matched*.csv`（arm 名带 `rep14sep`）：C10 epoch-200 成对差 st1–4 = −1.73 / −0.66 / +1.21 / +1.46，C100 = −0.11 / +1.20 / +0.89 / +1.00；epoch-20 C100 = −0.32 / +2.27 / +3.14 / +2.69。配置差异核实：`gate_ridge_ms` vs `gate_ridge_ms_beta0`（及 c100）仅 `loss.beta`；两者相对归档配置只多 `checkpoint_milestones: [20, 200]`。配对对象是同批 `gate1_20260914_ridge_ms(_c100)` seed 1–3（已完成）。本文档不含第 20 epoch 的因果或收敛速度结论。整组数值已导出，替换时应整组替换。

**须知（§5 详述）**：9 月 14 日的完整模型复现在同 GPU 家族上几乎复制了归档权重（4/6 单元 max|Δw| < 1.3e-2，探针值到小数点后四位相同），只有 2/6 是真正独立的新样本。β=0 复现里 CIFAR-100 seed 2、3 同样落在 A100 上并近似复制了归档权重（max|Δw| 9.9e-3 / 9.8e-3，SGD 值相同）；CIFAR-10 三个 seed（L40S）和 CIFAR-100 seed 1 是独立样本。
...
1. ~~ridge β=0 里程碑复现~~ 已于 15:40 完成并写入（§2）。
