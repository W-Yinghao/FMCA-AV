...
Snapshot: 2026-08-09 05:00 CEST. The experiment program is running and is not yet complete.
...
- E10: the first preserved child `930585` exposed the nonfinite guard. Moment promotion fixed low-precision covariance accumulation, but `20260809-043326_postfix-e10-complexity` showed the manual FP16 benchmark still poisoned weights because it omitted Lightning's GradScaler. Slurm CPU job `931155` validates the stricter numerics policy. Watcher `20260809-050058_postfix-e10-benchmark-chain-fp32moments` reruns complexity with autocast+GradScaler, reuses valid post-fix one-GPU DDP result `931076`, submits only the two-GPU point, and requires all corrected inputs before rendering. Earlier 16/20 tables remain superseded and unpublished.
